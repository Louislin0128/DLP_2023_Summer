...
def generate_Ground_truth(idx, save_root):
    Str = os.path.join(save_root, f'LAB4_dataset\\Demo_Test\\GT{idx}.csv')
    loaded_2D_mat = np.loadtxt(Str)
...
def calculate_PSNR(save_root, gen_image, idx):
    ground_truth = generate_Ground_truth(idx, save_root)[0]
...
class Dataset_Dance(torchData):
    def __init__(self, root, transform, mode='test', video_len=7, partial=1.0):
        ...
        data_num = len(glob(os.path.join(root ,'.\\Demo_Test\\*')))
